...
INFURA_URL = process.env.INFURA_MAINNET + process.env.INFURA_KEY;
mainnetProvider = new ethers.JsonRpcProvider(INFURA_URL);
gorliProvider = new ethers.JsonRpcProvider(
    process.env.INFURA_GOERLI + process.env.INFURA_KEY
);
...
mainnetProvider.getNetwork().then((network) => {
    console.log(network);
});
...
const blockDiff = async () => {
    mainnet = await mainnetProvider.getBlockNumber();
    goerli = await gorliProvider.getBlockNumber();
    console.log("Mainnet block number:", mainnet);
    console.log("Goerli block number:", goerli);
    console.log("Difference:", mainnet - goerli);
};
...
const checkBlockTime = async (providerName = "mainnet", blocks2check = 3) => {
    // JS Ternary Operator.
    let provider =
        providerName.toLowerCase() === "mainnet"
            ? mainnetProvider
            : gorliProvider;
...
const getBlock = async () => {
    ...
    console.log("Block hash:", block.transactions);
    console.log("Receipts root:", block.transactions[0]);
    return await block;
};
